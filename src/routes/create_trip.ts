...
export async function  createTrip(app: FastifyInstance) {
    app.withTypeProvider<ZodTypeProvider>().post('/trips', {
        ...
        const message = await mail.sendMail({
            ...
            subject: `Confirme sua viagem para ${destination}em ${formatStartDay}`,
            html: `
            <div style="font-family: sans-serif; font-size: 16px; line-height: 1.6;">
              <p>Você solicitou a criação de uma viagem para <strong>${destination}</strong> nas datas de <strong>${formatStartDay}</strong> até <strong>${formatEndDay}</strong>.</p>
              <p></p>
